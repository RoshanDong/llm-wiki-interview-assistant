...
const CHAT_BRIDGE_TIMEOUT: Duration = Duration::from_secs(120);
const API_CHAT_REQUEST_EVENT: &str = "llm-wiki://api-chat-request";
...
static CHAT_REQUEST_COUNTER: AtomicUsize = AtomicUsize::new(0);
...
static PENDING_CHAT_REQUESTS: OnceLock<Mutex<HashMap<String, mpsc::Sender<Value>>>> =
    OnceLock::new();
...
pub fn complete_chat_request(request_id: String, response: Value) -> Result<(), String> {
    let lock = PENDING_CHAT_REQUESTS.get_or_init(|| Mutex::new(HashMap::new()));
    let sender = lock
        .lock()
        .map_err(|_| "Chat request registry is unavailable".to_string())?
        .remove(&request_id)
        .ok_or_else(|| format!("Unknown or expired chat request: {request_id}"))?;
    sender
        .send(response)
        .map_err(|_| format!("Chat request receiver is closed: {request_id}"))
}
...
fn handle_chat(app: &AppHandle, project_id: &str, body: &str) -> ApiResponse {
    ...
    let message = parsed
        ...
        .trim();
    if message.is_empty() {
        return err_with_code(400, "Chat message cannot be empty", "empty_message");
    }

    let request_id = format!(
        "chat_{}_{}",
        now_millis(),
        CHAT_REQUEST_COUNTER.fetch_add(1, Ordering::Relaxed)
    );
    eprintln!(
        "[API Server] chat request started request_id={request_id} project_id={} path={}",
        project.id, project.path
    );
    match dispatch_chat_request(app, &request_id, &project, parsed) {
        Ok(response) => {
            let success = response.get("ok").and_then(Value::as_bool).unwrap_or(false);
            if success {
                eprintln!(
                    "[API Server] chat request completed request_id={request_id} project_id={}",
                    project.id
                );
                ok(response)
            } else {
                let status = response
                    ...
                    .unwrap_or(500);
                eprintln!(
                    "[API Server] chat request failed request_id={request_id} status={status} body={response}"
                );
                ApiResponse {
                    status,
                    body: response,
        ...
        Err(ChatBridgeError::Emit(message)) => {
            eprintln!(
                "[API Server] chat bridge dispatch failed request_id={request_id}: {message}"
            );
            err_with_code(500, message, "chat_bridge_unavailable")
        }
        Err(ChatBridgeError::Timeout) => {
            eprintln!("[API Server] chat request timed out request_id={request_id}");
            err_with_code(
                504,
                "Timed out waiting for the chat bridge",
                "chat_bridge_timeout",
            )
        }
    }
...
enum ChatBridgeError {
    Emit(String),
    Timeout,
}

fn dispatch_chat_request(
    app: &AppHandle,
    request_id: &str,
    project: &ProjectEntry,
    body: Value,
) -> Result<Value, ChatBridgeError> {
    let (tx, rx) = mpsc::channel();
    let lock = PENDING_CHAT_REQUESTS.get_or_init(|| Mutex::new(HashMap::new()));
    lock.lock()
        .map_err(|_| ChatBridgeError::Emit("Chat request registry is unavailable".to_string()))?
    ...
    if let Err(err) = app.emit(API_CHAT_REQUEST_EVENT, payload) {
        remove_pending_chat_request(request_id);
        return Err(ChatBridgeError::Emit(format!(
            "Failed to dispatch chat bridge event: {err}"
        )));
    }
    match rx.recv_timeout(CHAT_BRIDGE_TIMEOUT) {
        ...
        Err(mpsc::RecvTimeoutError::Disconnected) => {
            remove_pending_chat_request(request_id);
            Err(ChatBridgeError::Emit(
                "Chat bridge disconnected before responding".to_string(),
            ))
        }
    }
}

fn remove_pending_chat_request(request_id: &str) {
    if let Some(lock) = PENDING_CHAT_REQUESTS.get() {
...
fn err_with_code(status: u16, message: impl Into<String>, code: &'static str) -> ApiResponse {
    ApiResponse {
        status,
        body: json!({ "ok": false, "error": message.into(), "code": code, "status": status }),
    }
}
