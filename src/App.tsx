...
import { registerChatApiBridge, unregisterChatApiBridge } from "@/lib/chat-api-bridge"
...
function App() {
  ...
  useEffect(() => {
    ...
    startClipWatcher()
    let cancelled = false
    let disposeChatApiBridge: (() => void) | null = null
    registerChatApiBridge().then((dispose) => {
      if (cancelled) {
        dispose()
      } else {
        disposeChatApiBridge = dispose
      }
    }).catch((err) => {
      console.warn("[chat-api-bridge] failed to register:", err)
    })
    return () => {
      cancelled = true
      disposeChatApiBridge?.()
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const savedAsrConfig = await loadAsrConfig()
        if (savedAsrConfig) {
          useWikiStore.getState().setAsrConfig(savedAsrConfig)
        }
