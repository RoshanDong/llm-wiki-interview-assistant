...
describe("chat persistence — round-trip (new format)", () => {
  ...
  it("does not include temporary interview assistant state in chat persistence", async () => {
    useInterviewStore.getState().resetSession(1000)
    useInterviewStore.getState().addTranscriptSegment({
      id: "seg-secret",
      sessionId: "session-secret",
      speaker: "interviewer",
      text: "temporary transcript should not persist",
      startMs: 0,
      endMs: 500,
      confidence: 0.9,
      source: "microphone",
    ...
    useInterviewStore.getState().addDetectedQuestions([{
      id: "q-secret",
      sessionId: "session-secret",
      text: "temporary question should not persist?",
      sourceSegmentIds: ["seg-secret"],
      detectedAt: 1000,
      status: "pending",
      queuedAt: 1000,
      startedAt: null,
    ...
    await saveChatHistory(tmp.path, [makeConv("c1", "Chat")], [makeMsg("m1", "c1", "chat only")])
    const raw = await readFileRaw(`${tmp.path}/.llm-wiki/chats/c1.json`)

    expect(raw).toContain("chat only")
    expect(raw).not.toContain("temporary transcript should not persist")
    expect(raw).not.toContain("temporary question should not persist")
  })
})
