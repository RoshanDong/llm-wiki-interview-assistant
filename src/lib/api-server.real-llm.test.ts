...
interface ChatResponse extends ApiEnvelope {
  projectId: string
  conversation: {
    id: string
    title: string
...
describe.skipIf(!ENABLED)("local API v1 against real project", () => {
  ...
  it(
    "sends a standalone chat message and returns the completed turn",
    async (ctx) => {
      ensureServer(ctx)
      await requireUsableApi()
      const chatResp = await api<ChatResponse>(`/api/v1/projects/${PROJECT_ID}/chat`, {
        method: "POST",
        body: JSON.stringify({ message: "Answer briefly: what is this wiki about?" }),
      })
      expect(chatResp.status).toBe(200)
      expect(chatResp.body.ok).toBe(true)
      expect(chatResp.body.projectId).toBe(PROJECT_ID)
      expect(chatResp.body.conversation.id).toBeTruthy()
      expect(chatResp.body.userMessage).toMatchObject({
        role: "user",
        conversationId: chatResp.body.conversation.id,
      })
      expect(chatResp.body.assistantMessage).toMatchObject({
        role: "assistant",
        conversationId: chatResp.body.conversation.id,
      })
      expect(chatResp.body.assistantMessage.content.trim().length).toBeGreaterThan(0)
      expect(Array.isArray(chatResp.body.references)).toBe(true)
      expect(chatResp.body.historyLimit).toBeGreaterThan(0)
    },
    CHAT_TIMEOUT_MS,
  )

  it(
    "rejects empty chat messages with 400",
    async (ctx) => {
      ensureServer(ctx)
      await requireUsableApi()
      ...
      expect(chatResp.body.code).toBe("empty_message")
    },
    TEST_TIMEOUT_MS,
  )

  it(
    "returns 404 for chat requests to unknown project ids",
    async (ctx) => {
      ensureServer(ctx)
      await requireUsableApi()
      const chatResp = await api<ApiEnvelope>(
        "/api/v1/projects/00000000-0000-0000-0000-000000000000/chat",
        {
          method: "POST",
  ...
  it.runIf(process.env.API_EXPECT_CHAT_BRIDGE_UNAVAILABLE === "1")(
    "reports a structured error when the chat bridge is unavailable",
    async (ctx) => {
      ensureServer(ctx)
      await requireUsableApi()
      const chatResp = await api<ApiEnvelope>(`/api/v1/projects/${PROJECT_ID}/chat`, {
      ...
      expect([500, 504]).toContain(chatResp.status)
      expect(chatResp.body.ok).toBe(false)
      expect(["chat_bridge_unavailable", "chat_bridge_timeout"]).toContain(chatResp.body.code)
    },
    CHAT_TIMEOUT_MS,
  )
