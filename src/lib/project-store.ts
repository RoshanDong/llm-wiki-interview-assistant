...
import { DEFAULT_ASR_CONFIG } from "@/stores/wiki-store"
import type {
  ApiConfig,
  AsrConfig,
  EmbeddingConfig,
...
import type { InterviewProjectProfileSet, PromptTemplateSet } from "@/lib/interview-types"
...
const ASR_CONFIG_KEY = "asrConfig"
...
export function normalizeAsrConfig(config?: unknown): AsrConfig {
  const input = config && typeof config === "object"
    ? config as Partial<Record<keyof AsrConfig, unknown>>
    : {}
  const provider =
    input.provider === "volcengine-streaming" ||
  ...
  const responseFormat =
    ...
    input.responseFormat === "diarized_json"
      ...
      : DEFAULT_ASR_CONFIG.responseFormat
  const chunkingStrategy = input.chunkingStrategy === "none" ? "none" : DEFAULT_ASR_CONFIG.chunkingStrategy
  const rawEndpoint = typeof input.endpoint === "string" && input.endpoint.trim()
    ? input.endpoint.trim()
    : DEFAULT_ASR_CONFIG.endpoint
  const endpoint = normalizeAsrEndpoint(provider, rawEndpoint)
  const model = typeof input.model === "string" && input.model.trim()
    ? input.model.trim()
    : DEFAULT_ASR_CONFIG.model
  const packetMs = typeof input.packetMs === "number" && Number.isFinite(input.packetMs)
    ? Math.max(100, Math.min(200, Math.round(input.packetMs)))
    : DEFAULT_ASR_CONFIG.packetMs
  const endWindowSizeMs = typeof input.endWindowSizeMs === "number" && Number.isFinite(input.endWindowSizeMs)
    ? Math.max(200, Math.round(input.endWindowSizeMs))
    : DEFAULT_ASR_CONFIG.endWindowSizeMs
  const forceToSpeechTimeMs = typeof input.forceToSpeechTimeMs === "number" && Number.isFinite(input.forceToSpeechTimeMs)
    ? Math.max(1, Math.round(input.forceToSpeechTimeMs))
    : DEFAULT_ASR_CONFIG.forceToSpeechTimeMs

  return {
    provider,
    apiKey: typeof input.apiKey === "string" ? input.apiKey : DEFAULT_ASR_CONFIG.apiKey,
    endpoint,
    resourceId: typeof input.resourceId === "string" && input.resourceId.trim()
      ? input.resourceId.trim()
      : DEFAULT_ASR_CONFIG.resourceId,
    packetMs,
    sampleRate: 16000,
    enableNonstream: typeof input.enableNonstream === "boolean"
      ? input.enableNonstream
      : DEFAULT_ASR_CONFIG.enableNonstream,
    showUtterances: typeof input.showUtterances === "boolean"
      ? input.showUtterances
      : DEFAULT_ASR_CONFIG.showUtterances,
    enableSpeakerInfo: input.enableSpeakerInfo === true,
    endWindowSizeMs,
    forceToSpeechTimeMs,
    model,
    responseFormat,
...
function normalizeAsrEndpoint(provider: AsrConfig["provider"], endpoint: string): string {
  if (provider !== "volcengine-streaming") return endpoint
  if (isLegacyBatchAsrEndpoint(endpoint)) return DEFAULT_ASR_CONFIG.endpoint
...
function isLegacyBatchAsrEndpoint(endpoint: string): boolean {
  try {
...
export async function saveAsrConfig(config: AsrConfig): Promise<void> {
  const store = await getStore()
  await store.set(ASR_CONFIG_KEY, normalizeAsrConfig(config))
}

export async function loadAsrConfig(): Promise<AsrConfig | null> {
  const store = await getStore()
  const config = await store.get<unknown>(ASR_CONFIG_KEY)
  return config ? normalizeAsrConfig(config) : null
}
...
const INTERVIEW_PROMPT_TEMPLATES_KEY = "interviewPromptTemplates"

export async function saveInterviewPromptTemplates(templates: PromptTemplateSet): Promise<void> {
  const store = await getStore()
  await store.set(INTERVIEW_PROMPT_TEMPLATES_KEY, templates)
}

export async function loadInterviewPromptTemplates(): Promise<Partial<PromptTemplateSet> | null> {
  const store = await getStore()
  return (await store.get<Partial<PromptTemplateSet>>(INTERVIEW_PROMPT_TEMPLATES_KEY)) ?? null
}

const INTERVIEW_PROJECT_PROFILES_KEY = "interviewProjectProfiles"

export async function saveInterviewProjectProfiles(profiles: InterviewProjectProfileSet): Promise<void> {
  const store = await getStore()
  await store.set(INTERVIEW_PROJECT_PROFILES_KEY, profiles)
}

export async function loadInterviewProjectProfiles(): Promise<Partial<InterviewProjectProfileSet> | null> {
  const store = await getStore()
  return (await store.get<Partial<InterviewProjectProfileSet>>(INTERVIEW_PROJECT_PROFILES_KEY)) ?? null
}
