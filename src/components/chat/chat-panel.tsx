...
import { useChatStore, type MessageImage } from "@/stores/chat-store"
...
import { type ChatAgentEvent } from "@/lib/chat-agent"
import { sendStandaloneChatMessage } from "@/lib/chat-service"
...
export function ChatPanel() {
  ...
  const handleSend = useCallback(
    async (
      ...
      try {
        ...
        const response = await sendStandaloneChatMessage({
          ...
          request: {
            message: text,
            conversationId: chatState.activeConversationId ?? undefined,
            images,
            options: {
              useWebSearch: sendOptions.useWebSearch,
              useAnyTxtSearch: sendOptions.useAnyTxtSearch,
          ...
          onUserMessage: (message, conversations, messages) => {
            if (!isCurrentRun()) return
        ...
        if (!isCurrentRun()) return
        lastQueryPages = response.queryPages ?? []
        completed = true
        setAgentEvents([])
    ...
    [project, llmConfig, searchApiConfig, setStreaming, appendStreamToken, finalizeStream, maxHistoryMessages],
  )
