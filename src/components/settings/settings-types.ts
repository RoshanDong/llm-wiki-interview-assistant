...
import type { AsrChunkingStrategy, AsrProvider, AsrResponseFormat, AzureModelFamily, CloseBehavior, MineruModelVersion, ReasoningConfig, SourceWatchConfig } from "@/stores/wiki-store"
...
export interface SettingsDraft {
  ...
  // ASR / transcription for Interview Assistant
  asrProvider: AsrProvider
  asrApiKey: string
  asrEndpoint: string
  asrResourceId: string
  asrPacketMs: number
  asrEnableNonstream: boolean
  asrShowUtterances: boolean
  asrEnableSpeakerInfo: boolean
  asrEndWindowSizeMs: number
  asrForceToSpeechTimeMs: number
  asrModel: string
  asrResponseFormat: AsrResponseFormat
  asrChunkingStrategy: AsrChunkingStrategy
