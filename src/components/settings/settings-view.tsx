...
import { AsrSection } from "./sections/asr-section"
...
const CATEGORIES: Category[] = [
  ...
  { id: "asr", labelKey: "settings.categories.asr", icon: Mic },
...
function initialDraft(
  ...
  asr: ReturnType<typeof useWikiStore.getState>["asrConfig"],
  ...
  return {
    ...
    asrResourceId: asr.resourceId,
    asrPacketMs: asr.packetMs,
    asrEnableNonstream: asr.enableNonstream,
    asrShowUtterances: asr.showUtterances,
    asrEnableSpeakerInfo: asr.enableSpeakerInfo,
    asrEndWindowSizeMs: asr.endWindowSizeMs,
    asrForceToSpeechTimeMs: asr.forceToSpeechTimeMs,
    asrModel: asr.model,
    asrResponseFormat: asr.responseFormat,
    asrChunkingStrategy: asr.chunkingStrategy,
...
export function SettingsView() {
  ...
  const asrConfig = useWikiStore((s) => s.asrConfig)
  const setAsrConfig = useWikiStore((s) => s.setAsrConfig)
  ...
  const handleSave = useCallback(async () => {
    ...
    const newAsr = {
      provider: draft.asrProvider,
      apiKey: draft.asrApiKey.trim(),
      endpoint: draft.asrEndpoint.trim() || DEFAULT_ASR_CONFIG.endpoint,
      resourceId: draft.asrResourceId.trim() || DEFAULT_ASR_CONFIG.resourceId,
      packetMs: Math.max(100, Math.min(200, Math.round(draft.asrPacketMs || DEFAULT_ASR_CONFIG.packetMs))),
      sampleRate: 16000 as const,
      enableNonstream: draft.asrEnableNonstream,
      showUtterances: draft.asrShowUtterances,
      enableSpeakerInfo: draft.asrEnableSpeakerInfo,
      endWindowSizeMs: Math.max(200, Math.round(draft.asrEndWindowSizeMs || DEFAULT_ASR_CONFIG.endWindowSizeMs)),
      forceToSpeechTimeMs: Math.max(1, Math.round(
        draft.asrForceToSpeechTimeMs || DEFAULT_ASR_CONFIG.forceToSpeechTimeMs,
      )),
      model: draft.asrModel.trim() || DEFAULT_ASR_CONFIG.model,
      responseFormat: draft.asrResponseFormat,
      chunkingStrategy: draft.asrChunkingStrategy,
    }
    const newEmbed = {
    ...
    try {
      ...
      try {
        ...
        setAsrConfig(resultValue(persistedAsr, null) ?? asrConfig)
  ...
  const body = useMemo(() => {
    switch (active) {
      ...
      case "asr":
        return <AsrSection draft={draft} setDraft={setDraft} />
      case "embedding":
